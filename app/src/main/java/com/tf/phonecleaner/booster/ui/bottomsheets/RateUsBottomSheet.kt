package com.tf.phonecleaner.booster.ui.bottomsheets
...
import com.tf.phonecleaner.booster.databinding.BottomSheetRateUsBinding
...
class RateUsBottomSheet(
    ...
    private fun updateRating(rating: Int) {
        ...
        stars.forEachIndexed { index, star ->
            if (index < rating) {
                star.setImageResource(com.tf.phonecleaner.booster.R.drawable.ic_star)
                star.setColorFilter(ContextCompat.getColor(currentContext, R.color.holo_orange_light))
            } else {
                star.setImageResource(com.tf.phonecleaner.booster.R.drawable.ic_star_non_fil)
