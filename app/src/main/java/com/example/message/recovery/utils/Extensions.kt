...
fun applySavedAppLocale(savedLang: String) {
...
fun View.setClickWithTimeout(
    ...
    setOnClickListener { view ->
        ...
        Log.e("ClickEvent", "click_event: $eventName")
        ...
        if (!AdMobManager.isPremium && !SessionClickManager.hasShownPremiumThisSession) {
            ...
            if (isOnboardingComplete) {
                ...
                if (SessionClickManager.globalClickCount >= RemoteConfigManager.getAdRules().clickCountPremiumActivity &&
                    RemoteConfigManager.getAdRules().showPremiumActivityAfterThreeClick
                ) {
                    SessionClickManager.hasShownPremiumThisSession = true
                    AppNavigator.instance?.openPremiumFromIcon()
                }
            }
        }
...
fun View.startShakeAnimation(
    ...
    lifecycleScope.launch {
        val shakeAnim = android.view.animation.AnimationUtils.loadAnimation(
            this@startShakeAnimation.context,
            com.example.message.recovery.R.anim.shake
        )
        while (true) {
            kotlinx.coroutines.delay(delayMillis)
            this@startShakeAnimation.startAnimation(shakeAnim)
        }
    }
}
