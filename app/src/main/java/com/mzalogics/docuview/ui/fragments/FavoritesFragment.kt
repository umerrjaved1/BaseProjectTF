...
import com.mzalogics.docuview.databinding.FragmentFavoritesBinding
import com.mzalogics.docuview.model.DocumentItem
import com.mzalogics.docuview.ui.screens.DocumentViewerActivity
import com.mzalogics.docuview.ui.viewmodel.FavoritesEvent
import com.mzalogics.docuview.ui.viewmodel.FavoritesViewModel
...
class FavoritesFragment : Fragment() {
    ...
    private val adapter by lazy {
        DocumentListAdapter(
            onItemClick = { viewModel.onFavoriteClicked(it.id) },
        ...
        registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri: Uri? ->
            if (uri != null) {
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    ...
                    startActivity(
                        Intent(requireContext(), DocumentViewerActivity::class.java)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.favorites.collect { list ->
                    ...
                    binding.groupEmpty.visibility = if (list.isEmpty()) View.VISIBLE else View.GONE
    ...
    private fun showFileMoreMenu(anchor: View, item: DocumentItem) {
        ...
        val content = LayoutInflater.from(context).inflate(R.layout.bottom_sheet_file_actions, null)
        ...
        content.findViewById<TextView>(R.id.action_open).setOnClickListener {
            dialog.dismiss()
            viewModel.onFavoriteClicked(item.id)
        }
    ...
    private fun showFileDetailsSheet(item: DocumentItem) {
        val context = requireContext()
        val detailsDialog = BottomSheetDialog(context)
        val detailsView = LayoutInflater.from(context).inflate(R.layout.bottom_sheet_file_details, null)
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.events.collect { event ->
                    when (event) {
                        is FavoritesEvent.OpenDocument -> {
                            startActivity(
                                Intent(requireContext(), DocumentViewerActivity::class.java)
